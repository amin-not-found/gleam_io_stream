...
@external(erlang, "io_stream_ffi", "open")
@external(javascript, "./io_stream_ffi.mjs", "open")
fn ffi_open(
  path: String,
  modes: List(Mode),
) -> Result(Handle, #(String, String))

@external(erlang, "io_stream_ffi", "next_byte")
@external(javascript, "./io_stream_ffi.mjs", "next_byte")
pub fn next_byte(handle: Handle) -> Result(Int, #(String, String))

@external(erlang, "io_stream_ffi", "read_bytes")
@external(javascript, "./io_stream_ffi.mjs", "read_bytes")
pub fn read_bytes(
  handle: Handle,
  count: Int,
...
@external(erlang, "io_stream_ffi", "write_string")
@external(javascript, "./io_stream_ffi.mjs", "write_string")
pub fn write_string(
  handle: Handle,
  string: String,
) -> Result(Nil, #(String, String))

@external(erlang, "io_stream_ffi", "write_line")
@external(javascript, "./io_stream_ffi.mjs", "write_line")
pub fn write_line(
  handle: Handle,
  string: String,
) -> Result(Nil, #(String, String))

@external(erlang, "io_stream_ffi", "close")
@external(javascript, "./io_stream_ffi.mjs", "close")
pub fn close(handle: Handle) -> Result(Nil, #(String, String))

@external(erlang, "io_stream_ffi", "seek")
@external(javascript, "./io_stream_ffi.mjs", "seek")
pub fn seek(handle: Handle, position: Int) -> Result(Nil, #(String, String))

@external(erlang, "io_stream_ffi", "sync")
@external(javascript, "./io_stream_ffi.mjs", "sync")
pub fn sync(handle: Handle) -> Result(Nil, #(String, String))

@external(erlang, "io_stream_ffi", "stdin")
@external(javascript, "./io_stream_ffi.mjs", "stdin")
pub fn stdin() -> Handle

@external(erlang, "io_stream_ffi", "stdin_bin")
@external(javascript, "./io_stream_ffi.mjs", "stdin_bin")
pub fn stdin_bin() -> Handle

@external(erlang, "io_stream_ffi", "stdout")
@external(javascript, "./io_stream_ffi.mjs", "stdout")
pub fn stdout() -> Handle

@external(erlang, "io_stream_ffi", "stdout_bin")
@external(javascript, "./io_stream_ffi.mjs", "stdout_bin")
pub fn stdout_bin() -> Handle

@external(erlang, "io_stream_ffi", "stderr")
@external(javascript, "./io_stream_ffi.mjs", "stderr")
pub fn stderr() -> Handle

@external(erlang, "io_stream_ffi", "stderr_bin")
@external(javascript, "./io_stream_ffi.mjs", "stderr_bin")
...
pub fn map_system_error(error: #(String, String)) -> error.SystemError {
  ...
  case code {
    ...
    "EEXIST" -> error.FileAlreadyExists
    "EBADF" -> error.BadFileDescriptor
    "ENOTDIR" -> error.NotADirectory
    "EISDIR" -> error.IsADirectory
    "ENOTEMPTY" -> error.DirectoryNotEmpty
    "ENAMETOOLONG" -> error.NameTooLong
    "EMFILE" -> error.TooManyOpenFiles
    "ENFILE" -> error.SystemLimitReached
    "ENOSPC" -> error.NoSpaceLeft
    "EROFS" -> error.ReadOnlyFileSystem
    "EBUSY" -> error.ResourceBusy
    "EAGAIN" -> error.ResourceTemporarilyUnavailable
    "EINTR" -> error.Interrupted
    "EINVAL" -> error.InvalidArgument
    "EPIPE" -> error.BrokenPipe
    "EXDEV" -> error.CrossDeviceLink
    "ENODEV" -> error.NoSuchDevice
    "ENOTSUP" -> error.FunctionNotSupported
    "EOPNOTSUPP" -> error.FunctionNotSupported
    "EFBIG" -> error.FileTooLarge
    "ELOOP" -> error.SymbolicLinkLoop
    "EDQUOT" -> error.QuotaExceeded
    "ESTALE" -> error.StaleFileHandle
    ...
    "EMSGSIZE" -> error.MessageTooLarge
    _ -> error.Other(code: code, message: message)
  }
}

pub fn map_stream_error(error: #(String, String)) -> error.StreamError {
  let #(code, _) = error
  let code = string.uppercase(code)
  ...
  case code {
    ...
    _ -> error.System(map_system_error(error))
  }
}
